"""
modified from: https://github.com/rubenpt91/MP-DocVQA-Framework/blob/master/utils.py
"""
...
def parse_args():
    ...
    parser.add_argument('--only-keep-best', action='store_true', default=False, help='Whether to clean the results folder and only keep the best model.')
    ...
    parser.add_argument('--warmup-iterations', type=int, help='Warmup iterations.')
    parser.add_argument('--model-weights', type=str, help='Path to model weights.')
    parser.add_argument('--train-epochs', type=int, help='Number of training epochs.')

    parser.add_argument('--seed', type=int, help='Seed to allow reproducibility.')
    parser.add_argument('--save-dir', type=str, help='Path to save the model and results.')
    parser.add_argument('--project-name', type=str, help='Name of the project in wandb.', default='Documents')
    parser.add_argument('--wandb', action='store_true', help='Boolean to enable wandb logging.', default=False)

    parser.add_argument('--num-nodes', type=int, default=1, help='Number of available nodes/hosts')
    parser.add_argument('--node-id', type=int, default=0, help='Unique ID to identify the current node/host')
    parser.add_argument('--num-gpus', type=int, default=1, help='Number of GPUs in each node')
...
def time_stamp_to_hhmmss(timestamp, string=True):
    ...
    time = f"{hh:02d}:{mm:02d}:{ss:02d}" if string else [hh, mm, ss]
...
def save_model(model, epoch, experiment_name, update_best=False, **kwargs):
    save_dir = os.path.join(kwargs['save_dir'], 'checkpoints', f"{experiment_name}")
    model.save_pretrained(os.path.join(save_dir, f"model__{epoch}.ckpt"))

    tokenizer = model.processor if hasattr(model, 'processor') else model.tokenizer if hasattr(model, 'tokenizer') else None
    if tokenizer is not None:
        tokenizer.save_pretrained(os.path.join(save_dir, f"model__{epoch}.ckpt"))

    if hasattr(model, 'visual_embeddings'):
        model.visual_embeddings.feature_extractor.save_pretrained(os.path.join(save_dir, f"model__{epoch}.ckpt"))

    #save_yaml(os.path.join(save_dir, f"model__{epoch:d}.ckpt", "experiment_config.yml"), kwargs)
